...
/* Create new Lisp value of string type */
...
void lval_del(lval* v) {
  switch (v->type) {
    /* Do nothing special for number lvals */
  case LVAL_NUM: break;
  case LVAL_FUN:
    if (!v->builtin) {
      lenv_del(v->env);
...
lval* builtin_if(lenv* e, lval* a) {
  ...
  if (a->cell[0]->num) {
    /* If condition is true evaluate fist expression */
...
int main(int argc, char** argv) {
  ...
  if (argc >= 2) {
    ...
    for (int i = 1; i < argc; i++) {
      ...
      /* Pass to builtin load, get result */
      lval* x = builtin_load(e, args);

      /* If result is error, print it */
      if (x->type == LVAL_ERR) { lval_println(x); }
      lval_del(x);
    }
  }
